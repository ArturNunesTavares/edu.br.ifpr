...
class _CadastroState extends State<Cadastro> {
  ...
  _salvarAnotacao({Anotacao? anotacao}) async {
    ...
    if (anotacao == null) {
      Anotacao anotacao = Anotacao(texto,fotos, cor, porte);
      AnotacaoDAO().addAnotacao(anotacao, uID);
    } else {
      /*
      anotacao.texto = texto;
      anotacao.data = data.toString();
      anotacao.foto = foto;
...
/*
  _recuperarAnotacao(int id) async{
    var temp = await _db.recuperarAnotacao(id);
    Anotacao anotacaoTemp = Anotacao.fromMap(temp[0]);

    setState(() {
      _textoController.text = anotacaoTemp.texto!;
      _dataController.text = DateFormat("dd/MM/yyyy").format(DateTime.parse(anotacaoTemp.data!)).toString();
      _imagem = XFile(anotacaoTemp.foto!);
      _anotacao = anotacaoTemp;
    });
  }
*/
